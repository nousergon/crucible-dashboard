...
class TestScoreUncertainty:
    def test_lower_distance_higher_priority(self):
        # Scores nearer the rubric midpoint (3) have lower
        # uncertainty value = higher review priority.
...
class TestLoadRecentEvalArtifactsForReview:
    def test_ranks_by_uncertainty_lowest_first(self, mock_s3):
        ...
        batch = load_recent_eval_artifacts_for_review.__wrapped__(
            n=2, lookback_days=365, bucket="test-bucket",
        )
        # Uncertain (closer to midpoint) ranks first.
    ...
    def test_excludes_judge_skipped_artifacts(self, mock_s3):
        ...
        # Only the real (non-skipped) artifact lands.
        assert len(batch) == 1
    ...
    def test_stratifies_across_rubric_agent(self, mock_s3):
        # Two artifacts from the SAME (rubric, agent) — the picker
        # should take one and leave the other for the leftovers pass.
        # Then a third from a DIFFERENT (rubric, agent) gets pulled
        # in via the stratum pass even if its uncertainty is higher.
        a = _eval_artifact_payload(scores=[("d1", 3)])  # uncertainty 0
        a["judged_agent_id"] = "ic_cio"
        a["run_id"] = "A"
        b = _eval_artifact_payload(scores=[("d1", 3)])  # uncertainty 0
        b["judged_agent_id"] = "ic_cio"
        b["run_id"] = "B"
        c = _eval_artifact_payload(scores=[("d1", 4)])  # uncertainty 1 (worse)
        c["judged_agent_id"] = "sector_quant:tech"
        c["rubric_id"] = "eval_rubric_sector_quant"
        c["run_id"] = "C"
        ...
        by_key = {"a.json": a, "b.json": b, "c.json": c}
        mock_s3["fetch_json"].side_effect = lambda _b, k: by_key[k]

        batch = load_recent_eval_artifacts_for_review.__wrapped__(
            n=2, lookback_days=365, bucket="test-bucket",
        )
        # First two picks: one ic_cio + one sector_quant. Within ic_cio
        # the lower-uncertainty A wins. b stays in leftovers.
        ids = [art["run_id"] for art in batch]
        assert "A" in ids
        assert "C" in ids
        assert "B" not in ids
...
class TestSaveCalibrationReview:
    ...
    def test_first_write_uploads_jsonl_line(self, mock_s3):
        # No prior key — get_object raises → first-write path.
        ...
        assert "reviewed_at_utc" in parsed  # auto-stamped
    ...
    def test_failure_returns_false_no_raise(self, mock_s3):
        ...
        ok = save_calibration_review(
            {"review_id": "x"}, bucket="test-bucket",
        )
        assert ok is False

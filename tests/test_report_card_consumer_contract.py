...
CLOSED (alpha-engine-config-I7608): ``components/report_card_v2.py``'s
``render_attestation`` reads ``card["attestation"]`` and
``card["degraded_staleness"]`` / ``card["stale_tiles"]`` off the card root.
As of the pin below (nousergon-lib v0.124.65, nousergon-lib#324) the schema
declares ``attestation`` and ``degraded_staleness`` as REQUIRED and
``stale_tiles`` as an optional typed array — closing the drift where this
repo's own contract test could not yet check fields the dashboard has
depended on since the attestation banner shipped.
``TestConsumerAttestationFieldsAreDeclared`` below asserts this positively;
it previously carried ``strict=True`` xfail markers for these three fields
(scaffolding for a schema that did not exist yet, per PR710) — removed here
in the SAME PR as the pin bump, per config#7608's required merge order, so
the xpass window (an xfail that passes is a hard failure) never lands on
``main``.
"""
...
class TestLiveShapedPayloadValidates:
    """The v1 contract accepts a payload shaped like what the evaluator is
    actually emitting today, attestation fields included."""
...
class TestConsumerAttestationFieldsAreDeclared:
    """report_card_v2.py's render_attestation is the FIRST element rendered
    on every Report Card surface (sf-pipeline-policy.md Sec 2.3a rule 3) and
    reads card["attestation"], card["degraded_staleness"], card["stale_tiles"]
    off the root. nousergon-lib v0.124.65 (nousergon-lib#324) declares
    `attestation` and `degraded_staleness` as REQUIRED and `stale_tiles` as
    an optional typed array — these are now positive assertions, not xfail:
    a schema that stops declaring one of these fields must fail this class,
    not silently xpass an xfail marker (config#7608)."""
    ...
    def test_attestation_is_a_required_property(self, schema):
        assert "attestation" in schema["properties"]
        assert "attestation" in schema["required"]

    def test_degraded_staleness_is_a_required_property(self, schema):
        assert "degraded_staleness" in schema["properties"]
        assert "degraded_staleness" in schema["required"]

    def test_stale_tiles_is_a_declared_property(self, schema):
        # Optional (typed array), not required — report_card_v2.py handles
        # its absence, unlike attestation/degraded_staleness.
        assert "stale_tiles" in schema["properties"]

    def test_live_shaped_payload_carries_the_declared_fields(self):

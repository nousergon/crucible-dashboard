...
class TestLibVersionPin:
    ...
    def test_requirements_pins_lib_to_stable_tag(self):
        ...
        assert "@v0.26.0" in text, (
            "alpha-engine-lib should pin to v0.26.0 (adds "
            "``alpha_engine_lib.ec2_spot`` — Python CLI chokepoint for "
            "capacity-resilient spot launches that rotates "
            "``(instance_type × subnet)`` on capacity errors. **The "
            "spot launchers in alpha-engine-data, alpha-engine-predictor, "
            "and alpha-engine-backtester all invoke this CLI via "
            "``$LIB_PYTHON -m alpha_engine_lib.ec2_spot launch ...`` "
            "where $LIB_PYTHON defaults to this dashboard's venv.** "
            "ae-dashboard remains the SSM target for all 8 Saturday-SF "
            "spot states. Bumped from v0.25.0 2026-05-22 late evening "
            "after the dry-pass's Evaluator state hit "
            "InsufficientInstanceCapacity for c5.large in us-east-1f. "
            "Carries v0.25.0 ssm_log_capture transitively. Update this "

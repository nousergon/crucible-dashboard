...
class TestLibVersionPin:
    ...
    def test_requirements_pins_lib_to_stable_tag(self):
        ...
        assert "@v0.28.2" in text, (
            "alpha-engine-lib should pin to v0.28.2 (adds "
            "PublishResearchFailureImmediate + PublishPredictorFailureImmediate "
            "registry entries for the alpha-engine-data #300 fast-alert SF "
            "states — without v0.28.2 the registry-drift CI test fails on "
            "every `pytest tests/`). v0.28.1 added "
            "``alpha_engine_lib.pipeline_status`` (SF execution-state "
            "projection consumed by pages/25_Pipeline_Status.py for the "
            "pipeline-reporting-revamp arc, ROADMAP L3050) + a missing "
            "WaitForInstanceReady → StartExecutorEC2 WAIT_GROUPING entry. "
            "Bumped from v0.26.0 2026-05-24 — intermediate versions "
            "inherit transitively (v0.26.0 ec2_spot CLI; v0.27.0 trading-"
            "day-aware freshness predicates; v0.28.0 pipeline_status). "

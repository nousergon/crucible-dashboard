...
class TestLibVersionPin:
    ...
    2026-07-14 (config#2357, operator ruling 2026-07-13): crucible-dashboard
    adopted a uv-compiled lockfile. ``requirements.in`` is now the
    human-edited floors surface (where a literal ``>=``/``@vX.Y.Z`` tag
    reference is meaningful to assert against) and ``requirements.txt`` is
    the machine-generated, fully ``==``-pinned lock ``uv pip compile``
    produces from it — the literal-string assertions below moved from
    requirements.txt to requirements.in accordingly.
    """

    def test_requirements_txt_is_fully_pinned_lockfile(self):
        """Sanity check that requirements.txt is a compiled lock, not a
        copy of the floors file — every real dependency line is either
        ``==`` pinned or a ``git+`` URL pinned to a commit, never a bare
        ``>=`` floor."""
        text = (REPO_ROOT / "requirements.txt").read_text()
        assert "autogenerated by uv" in text, (
            "requirements.txt no longer looks like a uv-compiled lock — "
            "regenerate with `uv pip compile requirements.in --output-file "
            "requirements.txt`."
        )
        for line in text.splitlines():
            ...
            if not line or line.startswith("#"):
                continue
            if line.startswith("nousergon-lib"):
                assert "@" in line and "git+" in line, (
                    "nousergon-lib lock line must stay a git+ URL pin: " + line
                )
                continue
            assert "==" in line, (
                "requirements.txt line is not a pinned (==) dependency — "
                "the lock must be fully pinned: " + line
            )

    def test_requirements_in_pins_lib_to_stable_tag(self):
        text = (REPO_ROOT / "requirements.in").read_text()
        ...
        assert "@v0.124.88" in text, (
            "nousergon-lib should pin to v0.124.88 -- PublishAggregateCostsDegraded "
            ...
            "requirements.in on v0.124.86, so check_package_drift FAILED every "
            "deploy from #775's merge until 2026-08-25 -- crucible-dashboard#776 "
            "and #777 merged green and NEITHER reached the box, which kept the "
            "box_health.sh it had on 2026-08-22. CI stayed green throughout "
            "because CI installs the LOCKFILE and never compares it to "
            "requirements.in. Prior: v0.124.86 -- registers the weekly SF's "
            "WeeklyCoverageSweep / WeeklyCoverageSweepUnavailable Task states in "
            "nousergon_lib.pipeline_status.registry.STATE_TO_ARCHIVE_PAGE "
            "(nousergon-lib-PR350, alpha-engine-config-I8214). THIS REPO'S PIN IS "
            "THE GATE, not a follow-up: nousergon-data's CI installs "
            "crucible-dashboard/requirements.txt to run "
            "tests/test_pipeline_status_registry_drift.py against a candidate SF "
            "definition, so nousergon-data-PR1516's SF states cannot go green "
            "until this bump merges -- the pin here blocks a change in another "
            "repo, which is not obvious from either side. The same release also "
            "carries _extract_run_date's third source "
            "(alpha-engine-config-I8216): before it, every SCHEDULED weekly "
            "execution resolved to no cycle key and was DROPPED from its own "
            "cycle, so derive_cycle_verdict and the page-25 progress strip were "
            "reading cycles built from the operator reruns alone -- measured "
            "2026-08-22, the 2026-08-22 cycle showed 3 executions at 1/16 stages "
            "and hid the 02:00 scheduled run that reached 14/16. "
            "Prior: v0.124.83 -- publishes "
            "nousergon_lib.pipeline_status.classify_work() -> WorkOutcome, the "
            "three-way SUCCEEDED/SKIPPED/INCOMPLETE work verdict this repo's own "
            "status-blind pipeline-status call sites adopt in the same PR "
            "(alpha-engine-config-I8069, landed via nousergon-lib-PR347). "
            ...
            "Prior: v0.124.78 -- the first published release registering the "
            "Saturday SF's ScannerLeaderboard Task state and its "
            "PublishScannerLeaderboardDegraded / "
            "NotifyCompleteScannerLeaderboardDegraded companions "
            ...
            "alpha-engine-config-I7267), and v0.124.73's ResearchSelfTest weekly-SF "
            "state (nousergon-lib#332, config-I7726). Below this pin "
            "tests/test_pipeline_status_registry_drift.py fails on main: the drift "
            "detector correctly reports live SF states the installed registry does "
            "not know about, and a detector left red cannot report the NEXT drift "
            "(measured live 2026-08-19, alpha-engine-config-I7787: this exact pin "
            "gap failed crucible-dashboard#720's CI on an unrelated diff, "
            "because the guard above only asserts the pin is A tag, not that it is "
            "CURRENT enough to clear a drift that already shipped upstream). "
            "Prior: v0.124.70 -- the first published release registering the "
            "weekly SF's `RunScope` Task state in "
            "nousergon_lib.pipeline_status.registry.STATE_TO_ARCHIVE_PAGE "
            "(nousergon-lib#329, alpha-engine-config-I7620/I7649). "
            "Earlier: v0.124.68 -- nousergon-lib-PR327's merge-time autobump, "
            ...
            "it from the installed package instead of a sibling checkout at "
            "~/Development/crucible-backtester — a read whose verdict used to "
            "depend on another repo's working tree. (2) nousergon-lib#324 "
            "(alpha-engine-config-I7608, first published in v0.124.65): "
            "report_card.schema.json declares `attestation`, "
            "`degraded_attestation`, `degraded_staleness` and "
            "`degraded_pipeline_gates` as required plus `stale_tiles` as an "
            "optional array — the schema "
            "tests/test_report_card_consumer_contract.py checks against, "
            "closing the drift where report_card_v2.py::render_attestation "
            "read fields the pinned schema did not declare. "
            ...
            "'EXIT 1 — failing row(s)' line, because krepis.ssm_log_capture "
            "summarises a non-zero exit by quoting the LAST output line — with "
            "DEGRADED last, the weekly SF's 2026-08-15 failure named "
            "agent_decisions, a row that structurally cannot fail, and two "
            "sessions diagnosed the wrong one. (2) alpha-engine-config-I7420 "
            "(nousergon-lib#321): cost_telemetry is UNGATED again — the gate "
            "added in v0.124.60 named the retired Think Tank as the parquet's "
            "sole producer, and hours later config-I7179 made the weekly SF's "
            "own LLM stages producers on a live cadence, so the gate would "
            "have rendered a real capture failure as 'not a fault'. "
            "Earlier: v0.124.60 (alpha-engine-config-I7412, nousergon-lib#319): "
            "adds the GATED verdict to nousergon_lib.transparency — a row whose "
            "declared producer's EventBridge rule is DISABLED reports gated "
            "rather than fail; the mechanism stays, only cost_telemetry's use "
            "of it was scoped back. "
            ...
            "the venv here IS where all of this has to exist. "
            "Earlier: v0.124.45 (alpha-engine-config-I6919, "

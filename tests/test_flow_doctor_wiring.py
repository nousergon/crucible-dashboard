...
class TestLibVersionPin:
    ...
    def test_requirements_pins_lib_to_stable_tag(self):
        ...
        assert "@v0.28.1" in text, (
            "alpha-engine-lib should pin to v0.28.1 (adds "
            "``alpha_engine_lib.pipeline_status`` — SF execution-state "
            "projection consumed by pages/25_Pipeline_Status.py for the "
            "pipeline-reporting-revamp arc, ROADMAP L3050). v0.28.1 "
            "fixes a missing WaitForInstanceReady → StartExecutorEC2 "
            "WAIT_GROUPING entry caught by this repo's registry-drift "
            "CI test on first Phase-2 run. Bumped from v0.26.0 "
            "2026-05-24 — intermediate versions inherit transitively "
            "(v0.26.0 ec2_spot CLI invoked by spot launchers via "
            "$LIB_PYTHON -m alpha_engine_lib.ec2_spot launch from this "
            "dashboard's venv; v0.27.0 trading-day-aware freshness "
            "predicates). Update this test if the pin moves further "
            "forward."
        )

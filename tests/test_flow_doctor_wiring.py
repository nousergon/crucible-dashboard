...
class TestLibVersionPin:
    ...
    def test_requirements_pins_lib_to_stable_tag(self):
        ...
        assert "@v0.36.1" in text, (
            "alpha-engine-lib should pin to v0.36.1 (Option-D "
            "execution-picker substrate + AggregateCosts registry entry). "
            "Page 25 needs v0.36.0+ to filter SF executions by "
            "pipeline_role so smoke / recovery / operator-replay runs "
            "don't displace the canonical cadence run as 'most recent'; "
            "v0.36.1 adds the AggregateCosts registry entry needed for "
            "test_pipeline_status_registry_drift to pass. Bumped from "
            "v0.34.1 2026-05-25 — intermediate versions inherit "
            "transitively (v0.35.0 SSM dispatcher chokepoint, not "
            "imported here; v0.35.1 discriminated archive union + Scanner "
            "registry entry; v0.36.0 Option-D substrate; v0.36.1 "
            "AggregateCosts registry entry). The Pydantic V2 / 3.9 "

...
class TestLibVersionPin:
    ...
    def test_requirements_pins_lib_to_stable_tag(self):
        ...
        assert "@v0.32.0" in text, (
            "alpha-engine-lib should pin to v0.32.0 (adds the cost-telemetry "
            "Python 3.9 import fix — eval_type_backport conditional dep so "
            "Pydantic V2 can evaluate PEP 604 union annotations on the 3.9 "
            "EC2 venv this dashboard runs on). v0.32.0 is the load-bearing "
            "pin for this venv specifically: without it, ANY future module "
            "import that touches the cost module's Pydantic models "
            "(ModelMetadata, PriceCard, PriceTable, ToolFee, ToolFeeTable, "
            "DecisionArtifact) would raise ``TypeError: unsupported operand "
            "type(s) for |: type and NoneType`` at import time on 3.9. "
            "Bumped from v0.28.2 2026-05-25 — intermediate versions inherit "
            "transitively (v0.29.0 SecretsRedactingFilter auto-attach; "
            "v0.30.0 cost SDK adapter + packaged pricing; v0.31.0 server-tool "
            "fee tracking; v0.32.0 3.9-import fix). Update this test if "

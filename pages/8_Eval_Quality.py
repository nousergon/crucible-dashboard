...
#   Step 1 — operator scores the artifact BLIND (output + rubric only)
#   Step 2 — LLM judge's scores reveal; operator can revise + explain
# The Step-1 vs LLM kappa is the un-anchored calibration estimate.
# The Step-2 revisions surface systematic LLM-judge failure modes.
#
...
# coverage stays balanced rather than concentrating on one team.
#
# Each submitted review writes a JSONL line to
# `decision_artifacts/_calibration/{today}/reviews.jsonl` — durable
# substrate that a future Lambda / weekly evaluator stage computes
# Cohen's kappa from.
...
with tab_calibrate:
    ...
    st.caption(
        ...
        "scores are the un-anchored calibration anchor; revisions "
        "surface systematic LLM-judge failure modes. Each submitted "
        "review writes to `decision_artifacts/_calibration/{date}/"
        "reviews.jsonl`. ROADMAP L480."
    )
    ...
    if not batch:
        st.info(
            ...
            "Refresh after the next Saturday SF Research cycle to surface "
            "more candidates."
        )
    else:
        st.caption(
        ...
        for art in batch:
            ...
            with st.expander(
                ...
                # Always show the agent output context (sans LLM scores at Step 1).
                st.caption(
                ...
                if step == "blind":
                    ...
                    st.caption(
                        ...
                        "changes your mind. The blind-vs-LLM agreement is the load-bearing "
                    ...
                    for dim in dim_scores:
                        ...
                        final_score = st.slider(
                            f"Final score (after seeing LLM)",
                    ...
                    if submit_col.button("✓ Submit review", key=f"cal_submit__{rid}"):
                        ...
                        if ok:
                            ...
                            # Clear the cache so the queue re-loads without the
                    ...
                    if cancel_col.button(
                        ...
                        help="Reset to Step 1 — useful if you want to re-blind score "
                             "without seeing the LLM verdict.",
                    ):
    ...
    # ── Summary stats: total reviews + agreement ────────────────────────
    if reviewed_ids:
        st.divider()
        ...
        st.caption(
            ...
            "follow-up weekly evaluator stage once the corpus depth justifies "
            "it (~≥30 reviews per (rubric, dimension) cell). The persisted "
            "JSONL records carry blind_score, final_score, llm_score, and "
            "revision notes — everything kappa needs."
        )

...
def _review_id(eval_date: str, judged_agent_id: str, run_id: str, judge_model: str) -> str:
    """Stable opaque key for a single eval artifact under review.
...
def _score_uncertainty(dim_scores: list[dict], rubric_midpoint: float = 3.0) -> float:
    """Lower = higher review priority.

    Active-sampling heuristic: rank artifacts by the rubric-midpoint
    distance, summed across dimensions. Scores nearest the band
    boundary (mean → ``rubric_midpoint``) carry the highest information
    value per minute of operator review — those are the cases where the
    LLM judge is closest to indecisive between two scoring bands.

    Returns ``+inf`` when the artifact has no scored dimensions
    (kicked to the back of the queue rather than crashing the sort).
    """
    scores = [
        float(d.get("score"))
        for d in dim_scores or []
...
def load_recent_eval_artifacts_for_review(
    ...
    *,
    ...
    Excludes review_ids already in ``reviewed_ids`` so the queue
    self-shortens as the operator works through it. Stratified by
    rubric_id × judged_agent_id (top-1 per (rubric, agent) before
    second-pass) so coverage stays balanced rather than concentrating
    on one team.

    Loads the FULL artifact payload (not the long-format
    DataFrame) so the calibration UI can render the per-dimension
    reasoning + the overall_reasoning verbatim from what the judge
    saw. Best-effort — failures yield an empty list rather than
    raising.
    """
    bkt = bucket or _research_bucket()
    ...
    for d in _list_eval_dates(bkt):
        ...
        for key in _list_eval_keys_for_date(bkt, d):
            ...
            if not artifact or not isinstance(artifact, dict):
                continue
            # Skip judge-skipped artifacts — no per-dimension scores to
            # calibrate against (Layer-1 structural-skip is the right
            # behaviour for those, no human review needed).
    ...
    # Stratified pick: take the top-1 per (rubric_id, judged_agent_id)
    # first to guarantee dimension coverage, then fill remaining slots
    # by global uncertainty rank.
...
def _list_reviewed_keys(bucket: str) -> list[str]:
    """List every ``decision_artifacts/_calibration/{date}/reviews.jsonl``
    object — the per-date append-only review archives.
    """
    client = get_s3_client()
    paginator = client.get_paginator("list_objects_v2")
    keys: list[str] = []
...
def load_reviewed_ids(*, bucket: str | None = None) -> set[str]:
    """Return the set of ``review_id``s the operator has already
    submitted, across all per-date JSONL archives. Cache short — the
    UI submits a review and re-queries the queue on the same render
    cycle, so a 60s TTL is the right tradeoff.
    """
    import json
    ...
    for key in _list_reviewed_keys(bkt):
        ...
        for line in body.strip().split("\n"):
            ...
            try:
                ...
                # Tolerate corrupt lines — the operator can re-review;
...
def save_calibration_review(review: dict, *, bucket: str | None = None) -> bool:
    ...
    the new line, re-upload. Closes the L480 substrate gap by giving
    the operator's reviews a durable home that kappa-compute jobs
    can read.
    ...
    try:
        ...
        logger.info("[eval_loader] wrote calibration review %s → s3://%s/%s",
